...
from clusters_lib import read_clusters,plotting_tree,get_active_clusteres_data,get_all_children_full_name
...
#def plot_clusters_in_data_long_series(root, timeticks_cases, timeticks_series, alts_cases, alt_shape, time_shape, colors_list=[], file_name="clusters_in_data_long_series.png",output_dir="",hs=None,hvis=None,timeserie_cases=[],timeserie_series=[],not_final=None):
	#(root, timeticks_cases, alts_cases, alt_shape, time_shape, colors_list=[], file_name="clusters_in_data.png",output_dir="",hs=None,hvis=None,timeserie_cases=[],verbose=False):
	"""
	input:
		root - 				cluster id
		timeticks_cases - 	list of timeticks per case to use in the plot
	...
	"""
	levels = np.arange(200,800,100)
	
	#if(verbose):
	print "---------------------!!!!!! plot_clusters_in_data_long_series !!!!!!!"
	...
	#city = a["London"]
	#city = Astral.Location(info=("Porton Down", "UK",51.1167, 1.7000, "UK/GMT",157/FT_PER_METRE))
	...
	with pd.ExcelWriter(xlsxfile) as writer:
		...
		for p in range(0,plots_number):
			...
			#print "timeseries_shape[{}]".format(p)
			#print timeseries_shape[p]
			...
			for c in cases_list:
				...
				for l,lev in enumerate(levels):
					level_ind[int(l)] = np.argmin(np.absolute(alts-lev))
				
				#print "level_ind"	
				#print level_ind
				#print "c from cases_list is {}".format(c)
				#print "time_shape[{}]".format(c-1)
				#print time_shape[c-1]
				#print "timeserie_cases[{}].shape".format(c-1)
				#print timeserie_cases[c-1].shape
				#print timeserie_cases[c-1]
				#int_times = np.asarray(timeserie_cases[c-1]).astype(int)
				...
				#print "min(time_diff)"
				#print min(time_diff)
				#print int(max(time_diff)/count.most_common(1)[0][0])
				...
				#print "this_indxs.shape"
				#print this_indxs.shape
	
				#this_indxs = all_indxs[np.isin(all_data[:,-2], p_list[0])] - prev_max_index
				
				# get all altitudes for this day
				altitudes = alts_cases[c-1]
				
				# create an empty flat array to fill in with the data
				...
				#print "(int(max(time_diff)/count.most_common(1)[0][0])>1)"
				#print (int(max(time_diff)/count.most_common(1)[0][0])>1)
				
				# we start to take an action if there is a gap in the time series.
				if (int(max(time_diff)/count.most_common(1)[0][0])>1):
					
					#print "---time stamp is missing in "
					#print time_diff.index(max(time_diff))
					#print "---after"
					#print this_timeserie[time_diff.index(max(time_diff))]
					#print "---before"
					##print this_timeserie[time_diff.index(max(time_diff))+1]
					...
					Z_concat = np.concatenate((Zc[:,0:(ind+1)], np.zeros((alt_shape[c-1],len(list_toext)))*(np.nan)), axis=1) 
				...
				print "civil_twilight_this_day['dusk']"
				print civil_twilight_this_day['dusk']
				#print timeserie_cases[c-1]
				print "civil_twilight_this_day['dawn']"
				print civil_twilight_this_day['dawn']
				#print civil_twilight_this_day['dawn'].replace(tzinfo=None)
				if c==1:list_of_twilights = [civil_twilight_this_day]
				else:list_of_twilights.extend([civil_twilight_this_day])
				...
				if(len(ts) != time_shape[c-1]):
					Zc_with_filled_values = np.empty((alt_shape[c-1], len(ts))) * np.nan#(-9999)
					tiled_ts_mask = np.tile(ts_mask,(alt_shape[c-1],1))
					mZc = np.ma.array(Zc, mask=tiled_ts_mask)
					
				else:
					mZc = Zc
				
				#print "(c == cases_list[0])"
				#print (c == cases_list[0])
				if(c == cases_list[0]):
					Z = Zc
					mZ = mZc
			...
			if(colors_list == []):
				...
				# print cluster_colors
				# #if(len(ts) != time_shape[c-1]):
				# #	cluster_colors.set_bad(color = 'gray', alpha = 1.)
				# print "len(two_levels_list)"
				# print len(two_levels_list)
				# if (len(colors_list) <> len(two_levels_list)):
					# for c,a_color in enumerate(colors_list):
						# number_of_subclusters = len(re.findall(main_clusters[c], ':'.join(two_levels_list)))
			...
			x_lims = mdates.date2num([timeticks[0],timeticks[-1]])
			#print "x_lims = mdates.date2num([timeticks[0],timeticks[-1]])"
			#print '{}'.format([timeticks[0],timeticks[-1]])
			#print "x_lims"
			#print x_lims
			#print "series"
			#print series
			
			# xr_Z = xr.DataArray(Z,coords=[altitudes,series],dims=['altitude', 'time'])
			# print xr_Z 
			
			# fig1, ax11 = plt.subplots(sharey=False,figsize=(10*c,5))
	    
			# g = xr_Z.plot.imshow('time', 'altitude',robust=True,ax=ax11)
			# plt.draw()
			
			# plt.savefig("./test_png_image{}.png".format(p) , format='png',dpi=100)
			# plt.close("./test_png_image.png")
			...
			#plot_cluster_field_with_faam(Z,cm,cluster_norm,x_lims,np.nanmin(Z),np.nanmax(Z),new_labels,timeticks,savefile,alts=altitudes,hs=None,hvis=None,timeserie=timeserie_cases[c-1])#timeseries_series[p])
			
			#plot night data only
			#plot_cluster_field_night_hours(Z,cm,cluster_norm,x_lims,np.nanmin(Z),np.nanmax(Z),new_labels,timeticks,savefile,alts=altitudes,hs=None,hvis=None,timeserie=timeseries_series[p])
			
			
			
			#Counts = np.zeros(Z.shape)
			#Counts_night = np.zeros(Z_night.shape)
			#print range(0,clusters_number)
			df_cl1 = []
			df_cl1_night = []
			index = pd.DatetimeIndex(timeseries_series[p])
			# print "clusters_number = {}".format(clusters_number)
			# print "Z_night unique"
			# print np.unique(Z_night[~np.isnan(Z_night)])
			# print "Z unique"
			...
			if(clusters_number_night !=  clusters_number):
				...
				# print "cluster_colors_night"
				# print cluster_colors_night
				# print "cluster_colors"
				# print cluster_colors
				for j in np.unique(Z_night[~np.isnan(Z_night)]).astype(int):
					#print "j={}".format(j)
					#print cluster_colors[j]
					if (j==0):
						cluster_colors_night = [cluster_colors[j]]
						#print "cluster_colors_night"
						#print cluster_colors_night
					else:
						#print "j!=0 j ={}".format(j)
						#print cluster_colors_night
						#print cluster_colors[j]
						
						#print cluster_colors_night.extend(cluster_colors[j])
						cluster_colors_night.append(cluster_colors[j])#sns.color_palette("hls", clusters_number_night)
						#print "cluster_colors_night"
						#print cluster_colors_night
			else:
				cluster_colors_night = cluster_colors
			...
			fig_levels = plt.figure(figsize=(40,10))
			fig_levels,axa_levels = plt.subplots(3, 2)
			...
			for i in range(0,clusters_number):
				...
				#Counts_night_200 = np.zeros(Z_night[0:].shape)
				
				# print "set Counts_night to zero"
				# print "(~np.isnan(Z_night)).shape"
				# print (~np.isnan(Z_night)).shape
				# print "(np.nonzero(Counts_night))"
				# #print (np.nonzero(Counts_night)).shape
				# print (np.nonzero(Counts_night))
				if(np.nonzero(Counts_night)[0].size != 0): 
					tmp_Z_night = Z_night[Counts_night>0]
					tmp_Z_night =tmp_Z_night[(~np.isnan(tmp_Z_night))]
					
					#print "np.unique(tmp_Z_night)"
					#print np.unique(tmp_Z_night)
				...
				if(np.nonzero(Counts_night)[0].size != 0): 
					...
					# print "np.unique(tmp_Z)"
					# print np.unique(tmp_Z)
					# print "np.unique(tmp_Z_night)"
					# print np.unique(tmp_Z_night)
					
					# print "np.where(np.sum(Counts, axis=0))"
				# print np.where(np.sum(Counts, axis=0)!=0)
				# print "np.sum(Counts, axis=0).shape"
				# print np.sum(Counts, axis=0).shape	
				# print "np.where(np.sum(Counts_night, axis=0))"
				# print np.where(np.sum(Counts_night, axis=0)!=0)	
				
				# print "np.sum(Counts_night, axis=0).shape"
				# print np.sum(Counts_night, axis=0).shape
				...
				# print "index"
				# print index
				#print timeseries_series[p].shape
				#print timeserie_cases[c-1].shape
				...
				if(i==0):
					...
					for l, lev in enumerate(levels):
						...
						#else:
						#	margin_bottom_level[l] = lev_values
						#print "margin_bottom_level"
						#print margin_bottom_level
						
						if (l<3):
							#print "axa[{},0]".format(l)
							#print margin_bottom_level[l]
							#print "cluster_colors[{}]".format(i)
							#print cluster_colors[i]
							
							#print "df_night_levels_sum"
							#print df_night_levels_sum.values
							
							axa_levels[l,0].set_title("{} - {} m".format(prev_lev,lev))
							
							prev_lev = lev
							
							df_night_levels_sum.plot.bar(ax=axa_levels[l,0], stacked=True, bottom = margin_bottom_level[l], color=cluster_colors[i], label="f_cl{}".format(i), align='center')#cluster_colors_night[clc]
							if(l<2):
								for ticklabel in axa_levels[l,0].xaxis.get_ticklabels():
									ticklabel.set_visible(False)
								#if(l==0):df_cl1_night_levels = pd.DataFrame(data=df_night_levels_sum,index=l,columns=[cluster_labels[i]])#[cluster_labels[i]])#[new_labels[i]])
								#else:df_cl1_night_levels[cluster_labels[i]] = df_night_levels_sum
								...
								#axa_levels[l,0].xaxis.set_major_locator(locator)
								#axa_levels[l,0].xaxis.set_major_formatter(mdates.AutoDateFormatter(locator))
							...
							#print "df_night_levels_sum"
							#print df_night_levels_sum.values
							#axa_levels[l-3,1].set_title("level upper bound {}m".format(lev))
							
							axa_levels[l-3,1].set_title("{} - {} m".format(prev_lev,lev))
							
							prev_lev = lev
							
							
							if((l-3)<2):
								for ticklabel in axa_levels[l-3,1].xaxis.get_ticklabels():
									ticklabel.set_visible(False)
							else:
								#for ticklabel in axa_levels[l-3,1].xaxis.get_ticklabels():
								#	print "ticklabel"
								#	print ticklabel
								#locator = mdates.AutoDateLocator()
								#axa_levels[l-3,1].xaxis.set_major_locator(locator)
								#axa_levels[l-3,1].xaxis.set_major_formatter(mdates.AutoDateFormatter(locator))
								#axa_levels[l-3,1].xaxis.set_major_formatter(date_format)
								date_form = mdates.DateFormatter("%m-%d")
								axa_levels[l-3,1].xaxis.set_major_formatter(date_form)
								#.xaxis.set_tick_params(rotation=30, labelsize=8)
	
							df_night_levels_sum.plot.bar(ax=axa_levels[l-3,1], stacked=True, bottom = margin_bottom_level[l], color=cluster_colors[i], label="f_cl{}".format(i), align='center')#cluster_colors_night[clc]
							
							axa_levels[l-3,1].get_legend().remove()
							
						margin_bottom_level[l] = lev_values.flatten()
						#df_cl1_night_levels[cluster_labels[i]] = lev_values
					...
					#	if (l==0) :df_night_levels = pd.DataFrame(data=np.sum(Counts_level[l], axis=0),index=index,columns=[lev])#[cluster_labels[i]])#[new_labels[i]])
					#	else:
					#		another_column = pd.DataFrame(data=np.sum(Counts_level[l], axis=0),index=index,columns=[lev])#[cluster_labels[i]])#[new_labels[i]])
					#		df_night_levels = pd.concat([df_night_levels,another_column], axis=1, sort=False).reindex(df_night_levels.index)#.merge(df_cl2)#
					
					
					
					#print "after concat  df_cl1"
					#print df_cl1.describe
					print "after concat  df_cl1_night"
					print df_cl1_night.describe	
					print "l, lev in enumerate(levels)"
					print list(enumerate(levels))
					...
					for l, lev in enumerate(levels):
						...
						print "margin_bottom_level[{}].shape".format(l)
						print margin_bottom_level[l].shape
						print margin_bottom_level[l]
						
						print "lev_values.shape"
						print lev_values.flatten().shape
						print lev_values.flatten()
							
						
						
						print axa_levels.shape
						if (l<3):
							print "axa[{},0]".format(l)
							print margin_bottom_level[l]
							print "cluster_colors[{}]".format(i)
				...
				# print df_to_print2.columns.values
				# print "df_to_print2[{}]".format(i)
				# print df_to_print2[i]
				# print "df_to_print2[i].values"
				# print df_to_print2[i].values
				# print "tmp_Z_night"
				# print tmp_Z_night
				...
				if(i in tmp_Z_night):
					...
					#print ""
					df_to_print2[i].plot.bar(ax=axa, stacked=True, bottom = margin_bottom, color=cluster_colors[i], label="{}".format(i), align='center')#cluster_colors_night[clc]
					print "margin_bottom"
					print margin_bottom
			...
			#print len(list_of_twilights)
			#print cases_list
			#for case in range(len(list_of_twilights)):
			#	#this_timeserie = timeserie_cases[case-1]
			#	print "list_of_twilights[{}-1]".format(case)
			#	print len(list_of_twilights[case-1])
			...
			df_to_print1.plot.bar(stacked=True,ax=axa2,colormap=cm, align='center')
			plt.xticks(rotation=10)
			plt.draw()
...
def read_input_data(qvp_file,field_list,upper_indx,count_thr,alts,X,Y,pd_X,field_lables_list,case,verbose=False,debug=True,sys_phi_list=[]):
	"""
...
def main():
	...
	field_list = ['dBZ','ZDR','RhoHV','KDP_UKMO'] #,'PhiDP','KDP_UKMO'
	field_lables_list = ['dBZ','ZDR','RhoHV','KDP']#,'PhiDP','KDP'
	...
	(pd_X,timeticks_cases,timeticks_series,alts_cases,alt_shape,time_shape,timeseries_shape,indexes_cases,indexserie_series,sys_phi_list,hs_cases,hvis_cases,timeserie_cases,timeseries_series,h_list,start_end_series)= read_long_series(cases_file,qvp_directory_name,field_lables_list,field_list,count_thr,max_height,pd_X,elev,plot=True,output_dir=output_dir,verbose=verbose,debug=debug,trap_interval=None)		
																																																	#(cases_file,qvp_directory_name,field_lables_list,field_list,count_thr,max_height,pd_X,elev,plot=True,output_dir=None,verbose=False,debug=False)
	...
	for i in np.unique(df['cluster']):
		...
		for level in np.unique(a_cluster['height']):
			...
			#print a_level.describe()
			#print a_level.describe()['dBZ']['count']
			counts.append(a_level.describe()['dBZ']['count'])
		...
		"dBZ" : dBZ, "ZDR" : ZDR, "RhoHV" : RhoHV, "KDP" : KDP,
		"dBZ_std" : dBZ_std, "ZDR_std" : ZDR_std, "RhoHV_std" : RhoHV_std, "KDP_std" : KDP_std})
		labels = ['Counts',"$height",r'$\mathrm{Z_{H}}$ (dBZ)',
		r'$\mathrm{Z_{DR}}$ (dB)',r'$\mathrm{\rho_{HV}}$', 
	...
	c_clusters = pd.DataFrame({'labels': cluster_labels,'dBZ': c_dBZ,'ZDR': c_ZDR,'RhoHV': c_RhoHV,'KDP': c_KDP})
	...
	days_clusters = pd.DataFrame({'clusters': days_c_clusters,'dBZ': days_c_dBZ,'ZDR': days_c_ZDR,'RhoHV': days_c_RhoHV,'KDP': days_c_KDP,'days':days})
	print "days_clusters"
	...
	print '%d, %d' %(X0,X1)
	qvpp.Plot_standard_6(qvp_file,ver_file,timeticks,output_dir,faam_file,savefile=savefile,x0=X0,x1=X1,count_thr=100,hmin=0,hmax = 1500,hs=hs)
