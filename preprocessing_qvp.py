...
import numpy as np
import copy
#import pyart.retrieve.kdp_proc as kdp_proc
...
# print sys.path
# print os.environ
# os.environ['PYTHONPATH']="$HOME/Desktop/NCAS/QVP/nrt/"
# sys.path.insert(0,os.environ['PYTHONPATH'])

# if 'PYTHONPATH' in sorted(os.environ) is False:
...
# print 'PYTHONPATH' in sorted(os.environ)
...
def preprocssing(radar):
    ...
    #attenuation_correction(radar)
    
    #print "!!!! starting kdp calculation"
    #print 'ZDR'
    #print radar.fields['ZDR']['data'].size - np.count_nonzero(np.isnan(radar.fields['ZDR']['data']))
    #print radar.fields['ZDR']['data'].size - np.ma.count_masked(radar.fields['ZDR'])
    
    psidp_field = 0.632 * (copy.deepcopy(radar.fields['ZDR']['data'])) ** 1.71
    #print "psidp_field.shape {}".format(psidp_field.shape)
    
    radar.add_field_like('KDP', 'uPsiDP', psidp_field)
    
    # commented out 18.07.2018
    #filtered_psidp_field = kdp_proc.filter_psidp(radar, psidp_field='uPsiDP', rhohv_field='RhoHV', minsize_seq=3, median_filter_size=7, thresh_rhohv=0.65)
    #radar.add_field_like('KDP', 'PsiDP',filtered_psidp_field)
    
    
    
    #print "psidp"
    #print radar.fields['PsiDP']['data'].size - np.count_nonzero(np.isnan(radar.fields['PsiDP']['data']))
    #print radar.fields['PsiDP']['data'].size - np.ma.count_masked(radar.fields['PsiDP']['data'])
    #print "uKDP"
    #print radar.fields['uKDP']['data'].size - np.count_nonzero(np.isnan(radar.fields['uKDP']['data']))
    #print radar.fields['uKDP']['data'].size - np.ma.count_masked(radar.fields['uKDP']['data'])
    #print "uPhiDP"
    #print radar.fields['uPhiDP']['data'].size - np.count_nonzero(np.isnan(radar.fields['uPhiDP']['data']))
    #print radar.fields['uPhiDP']['data'].size - np.ma.count_masked(radar.fields['uPhiDP']['data'])
    
    #print "KDP_VULPIANI"
    #(kdp_dict2,phidp_dict2) = kdp_proc.kdp_vulpiani(radar, phidp_field='uPhiDP', psidp_field='PsiDP', kdp_field='uKDP', band="X")
    #radar.add_field_like('KDP', 'KDP_VULPIANI', kdp_dict2['data'])
    #print "KDP_VULPIANI"
    #print radar.fields['KDP_VULPIANI']['data'].size - np.count_nonzero(np.isnan(radar.fields['KDP_VULPIANI']['data']))
    #print radar.fields['KDP_VULPIANI']['data'].size - np.ma.count_masked(radar.fields['KDP_VULPIANI']['data'])
    #print radar.fields['KDP_VULPIANI']
    
    #print "KDP_VULPIANI is done"
    
    #phidp_ncas(radar,phidpfield='uPhiDP')
    
    #print "KDP_SCHNEEBELI"
    #(kdp_dict1,kdp_stdev_dict,phidp_dict1) = kdp_proc.kdp_schneebeli(radar, phidp_field='uPhiDP', psidp_field='PsiDP', kdp_field='uKDP',band='X')
    #print "kdp_dict"
    #print kdp_dict
    
    #print kdp_dict['data']
    #radar.add_field_like('KDP', 'KDP_SCHNEEBELI', kdp_dict['data'])
    

    #try:
        #print "kdp_ukmo(radar) where radar = %s" %radar
        #kdp_ukmo(radar)
